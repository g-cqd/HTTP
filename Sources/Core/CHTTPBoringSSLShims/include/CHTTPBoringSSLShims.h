...
/// backed by an (initially empty) registry that later ``CHTTPBoringSSLShims_add_sni_context`` registrations
...
/// ``CHTTPBoringSSLShims_enable_sni`` first). The registry takes its own reference, so the caller may free
