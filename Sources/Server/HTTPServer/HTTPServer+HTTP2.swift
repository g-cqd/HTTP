...
//
//  Bounded intake (2026-07-31 audit, finding 3): the mailbox carries payload-free tickets and the octets
//  themselves travel through a lossless, byte-watermarked `BoundedByteChannel`. The mailbox may stay
//  `.unbounded` because it is then provably bounded — an inbound ticket is 1:1 with a queued chunk, which
//  the channel caps. Previously the reader yielded octets straight into the unbounded mailbox in an
//  unconditional loop while its sole consumer also did protocol and application-adjacent work, so a peer
//  sending invalid or flow-control-exempt traffic could outpace it and grow memory without limit.
//
//  Per-wakeup handling lives in HTTPServer+HTTP2Wakeups.swift and the reader in
//  HTTPServer+HTTP2Reader.swift; the consumer's state is HTTP2ConnectionState. Before that split this
//  function was cyclomatic complexity 29 with a 134-line closure, against configured limits of 12 and 60.
//
...
extension HTTPServer {
    /// Drives the sans-I/O ``HTTP2Connection`` over `connection` for the connection's whole life.
    ///
    /// Feed octets → events → respond → flush, until EOF, a timeout, or a connection-level protocol
    ...
    func serveHTTP2(
        ...
        await withDiscardingTaskGroup { group in
            ...
            // comment). Times its receives against the ORIGINAL `deadline` parameter.
