...
public struct ConditionalRequestMiddleware: HTTPMiddleware {
    ...
    public func respond(
        ...
        guard isCacheable(request, response) else { return response }
        ...
        guard matches(request.headerFields.values(for: .ifNoneMatch), etag) else { return response }
    ...
    private func matches(_ ifNoneMatch: [String], _ etag: String) -> Bool {
        ...
        for value in ifNoneMatch {
            for element in value.split(separator: ",") {
                ...
                if candidate == "*" || Self.opaque(candidate) == target { return true }
            }
        }
        return false
    }
