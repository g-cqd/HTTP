...
//  state or trailers without END_STREAM is a connection error (PROTOCOL_ERROR), while a frame on a
//  closed stream is a stream error (STREAM_CLOSED).
//
    ...
    public mutating func receiveHeaders(endStream: Bool) throws(HTTP2Error) {
        ...
        case .open:
            guard endStream else {
                throw .connection(.protocolError, "trailers must set END_STREAM")
            }
