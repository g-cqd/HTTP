...
- **Status:** Accepted (HTTP/1.1 incremental; HTTP/2 + HTTP/3 buffered-as-stream, true-incremental staged)
...
- **HTTP/2 + HTTP/3 — buffered-as-stream (v1).** The sans-I/O engines already receive the whole body
  (bounded by the per-route limit) before surfacing the `.request` event; for a streaming route the
  server wraps those bytes as a one-shot `HTTPRequestBodyStream` (`HTTPServer.requestBody(_:for:)`). The
  handler API is **uniform across protocols** — `.stream` works everywhere — but on h2/h3 the bytes are
  not yet delivered incrementally off the wire.
...
- **Why h1 is truly incremental and h2/h3 are not (yet).** On HTTP/1.1 the server *owns* the read loop,
  so incremental delivery is a local change. On HTTP/2 the sans-I/O engine buffers DATA and the single
  multiplexed serve loop replenishes the flow-control window *as it buffers*; truly incremental,
  back-pressured delivery requires splitting the engine `Event` into `requestHead`/`requestBodyChunk`/
  `requestEnd` and gating window replenishment on handler consumption (the loop must never block on the
  handler, or it cannot read the `WINDOW_UPDATE` that unblocks the connection — the deadlock the response
  pump already guards against). That is a contained but invasive engine change best landed on its own,
  one protocol at a time, suite green after each. HTTP/3's independent QUIC streams make it the easier of
  the two and the natural first step.
...
- `.stream` + `Route.streamingBody()` work end-to-end on **all three protocols**; HTTP/1.1 delivers
  incrementally, HTTP/2 + HTTP/3 deliver the (limit-bounded) body wrapped as a stream.
- **Caveat — streamed-body errors surface as truncation, not status.** Once a streaming response's head
  ...
  abnormally.
- **Follow-ups** (tracked, not in this slice): the h2/h3 incremental `Event` split + server bridge for
  true wire-level streaming, and the request-direction back-pressure handoff (h1 first, then h2/h3).
