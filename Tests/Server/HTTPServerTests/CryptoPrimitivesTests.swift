...
//  HTTPServerTests
//
//  Known-answer vectors for the pure-Swift session-cookie crypto, independent of any crypto library:
//  FIPS 180-4 for SHA-256 and RFC 4231 for HMAC-SHA256. These lock in byte-correctness (a measured
//  decision kept these pure-Swift rather than pulling swift-crypto/BoringSSL into the dependency-light
//  core — same output, +467 ns/request, smaller attack surface). The constant-time comparison is
//  exercised too.
//
...
@Suite("Pure-Swift session crypto — known-answer vectors")
struct CryptoPrimitivesTests {
    ...
    /// Lowercase hex, no separators — Foundation-free so the test matches the core's no-Foundation posture.
