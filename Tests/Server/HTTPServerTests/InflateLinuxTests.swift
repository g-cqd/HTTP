...
//  HTTPServerTests
//
//  Exercises the Linux inbound decoders (InflateLinux over the in-house HTTPDeflate codec): a gzip
//  body round-trips through encode → decode, and a body that would decode past the cap fails closed
//  (the decompression-bomb defense, CWE-409). Gated `#if !canImport(Compression)` — the Linux build
//  graph; on Darwin inbound decode is Apple's Compression, exercised by DecompressionMiddlewareTests.
//
    ...
    @Suite("Linux inbound decompression (in-house HTTPDeflate)")
    struct InflateLinuxTests {
        ...
        func gzipInbound() throws {
            let coded = try #require(Gzip.compress(original), "the codec must encode")
